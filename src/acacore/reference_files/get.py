...
from acacore.models.reference_files import Action, CustomSignature, MasterConvertAction
...
def get_actions(*, cache: bool = True) -> dict[str, Action]:
    ...
    return TypeAdapter(dict[str, Action]).validate_python(
...
def get_custom_signatures(*, cache: bool = True) -> list[CustomSignature]:
    ...
    return TypeAdapter(list[CustomSignature]).validate_python(
        _get_yaml(url) if cache else _get_yaml.__wrapped__(url)
    )
